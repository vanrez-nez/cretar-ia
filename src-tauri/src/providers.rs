...
pub async fn save_provider_config_override(
    ...
    validate_role(role)?;
    validate_override_object(&override_config)?;
    let base = load_config_json(pool, "providers", provider_id).await?;
    let schema = load_schema_json(pool, "providers", provider_id).await?;
    let mut effective = base;
    merge_config_values(&mut effective, &override_config);
    validate_json(&schema, &effective, &format!("provider '{provider_id}' effective config"))?;
    save_override(pool, "provider_config_overrides", role, "provider_id", provider_id, &override_config).await
}
...
pub async fn save_model_config_override(
    ...
    validate_role(role)?;
    validate_override_object(&override_config)?;
    let base = load_config_json(pool, "models", model_id).await?;
    let schema = load_schema_json(pool, "models", model_id).await?;
    let mut effective = base;
    merge_config_values(&mut effective, &override_config);
    validate_json(&schema, &effective, &format!("model '{model_id}' effective config"))?;
    save_override(pool, "model_config_overrides", role, "model_id", model_id, &override_config).await
}

pub async fn save_model_provider_config_override(
    pool: &SqlitePool,
    role: &str,
    model_id: &str,
    override_config: Value,
    ...
    validate_override_object(&override_config)?;
    let row = sqlx::query(
        "SELECT p.config_json AS config_json, p.config_schema_json AS config_schema_json
         FROM models m
         JOIN providers p ON p.id = m.provider_id
         WHERE m.id = $1 AND m.role = $2",
    )
    .bind(model_id)
    .bind(role)
    .fetch_one(pool)
    .await
    .with_context(|| format!("loading provider config for model '{model_id}'"))?;
    let base = parse_json_column(&row, "config_json")?;
    let schema = parse_json_column(&row, "config_schema_json")?;
    let mut effective = base;
    merge_config_values(&mut effective, &override_config);
    validate_json(&schema, &effective, &format!("model '{model_id}' provider effective config"))?;
    save_override(pool, "model_provider_config_overrides", role, "model_id", model_id, &override_config).await
}

pub async fn reset_provider_config_override(pool: &SqlitePool, role: &str, provider_id: &str) -> Result<()> {
    validate_role(role)?;
    delete_override(pool, "provider_config_overrides", role, "provider_id", provider_id).await
}

pub async fn reset_model_config_override(pool: &SqlitePool, role: &str, model_id: &str) -> Result<()> {
    validate_role(role)?;
    delete_override(pool, "model_config_overrides", role, "model_id", model_id).await
}

async fn load_provider_override(pool: &SqlitePool, role: &str, provider_id: &str) -> Result<Value> {
    load_override(pool, "provider_config_overrides", role, "provider_id", provider_id).await
}

async fn load_model_provider_override(pool: &SqlitePool, role: &str, model_id: &str) -> Result<Value> {
    load_override(pool, "model_provider_config_overrides", role, "model_id", model_id).await
}

async fn load_model_override(pool: &SqlitePool, role: &str, model_id: &str) -> Result<Value> {
    load_override(pool, "model_config_overrides", role, "model_id", model_id).await
}
...
async fn load_override(
    ...
    let query = format!("SELECT config_json FROM {table} WHERE role = $1 AND {id_column} = $2");
    let row = sqlx::query(&query)
        .bind(role)
        .bind(id)
        .fetch_optional(pool)
        .await
        .with_context(|| format!("loading {table} override for role '{role}' id '{id}'"))?;
    let Some(row) = row else {
...
async fn save_override(
    ...
    {
        return delete_override(pool, table, role, id_column, id).await;
    }

    let now = chrono::Utc::now().to_rfc3339();
    let query = format!(
        "INSERT INTO {table} (role, {id_column}, config_json, created_at, updated_at)
         VALUES ($1, $2, $3, $4, $4)
         ON CONFLICT(role, {id_column}) DO UPDATE SET
    ...
    sqlx::query(&query)
        .bind(role)
        .bind(id)
        .bind(serde_json::to_string(override_config).context("serializing override config")?)
        .bind(now)
        .execute(pool)
        .await
        .with_context(|| format!("saving {table} override for role '{role}' id '{id}'"))?;
    Ok(())
}
...
async fn delete_override(
    ...
    let query = format!("DELETE FROM {table} WHERE role = $1 AND {id_column} = $2");
    sqlx::query(&query)
        .bind(role)
        .bind(id)
        .execute(pool)
        .await
        .with_context(|| format!("deleting {table} override for role '{role}' id '{id}'"))?;
    Ok(())
}

async fn load_schema_json(pool: &SqlitePool, table: &str, id: &str) -> Result<Value> {
    let query = match table {
        "providers" => "SELECT config_schema_json FROM providers WHERE id = $1",
        "models" => "SELECT config_schema_json FROM models WHERE id = $1",
        other => return Err(anyhow!("unsupported schema table '{other}'")),
    };
    let row = sqlx::query(query)
        .bind(id)
        .fetch_one(pool)
        .await
        .with_context(|| format!("loading {table} schema '{id}'"))?;
    parse_json_column(&row, "config_schema_json")
}
