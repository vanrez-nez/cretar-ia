...
pub fn migrate(raw: &str) -> Result<Value> {
    ...
    if !pipeline.contains_key("debounce_ms") {
        ...
        {
            pipeline.insert("debounce_ms".to_string(), value.clone());
        }
    }
    ...
    if !pipeline.contains_key("hotkey_queue_capacity") {
        ...
        {
            pipeline.insert("hotkey_queue_capacity".to_string(), value.clone());
        }
    }
    ...
    if !pipeline.contains_key("worker_queue_capacity") {
        ...
        {
            pipeline.insert("worker_queue_capacity".to_string(), value.clone());
        }
    }
    ...
    if !pipeline.contains_key("max_recording_duration_secs") {
        ...
        {
            pipeline.insert("max_recording_duration_secs".to_string(), value.clone());
        }
    }
